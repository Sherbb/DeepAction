...
public class DeepBehavior
{
    ...
    [HideInInspector]
    public ActionEntity parent;

    //resources require to TRIGGER the behavior. This is where you put the spell cost. Can be set by actions.
    public Dictionary<string,float>resourcesToTrigger = new Dictionary<string, float>();
    ...
    [HideInInspector]
    public ActionEntity behaviorTarget;
    public bool Trigger(Vector3 point,Vector3 direction,ActionEntity target)
    {
        foreach(string s in resourcesToTrigger.Keys)
        {
            if (!parent.resources.ContainsKey(s))
            {
                Debug.LogError(parent.gameObject.name + "Does not have the resource: " + s);
            }
            else
            {
                if (parent.resources[s].GetValue() < resourcesToTrigger[s])
                {
                    return false;
                }
            }
        ...
        foreach(string s in resourcesToTrigger.Keys)
        {
            parent.resources[s].TryToConsume(resourcesToTrigger[s]);
        }
    ...
    public bool Trigger(ActionEntity target){return Trigger(Vector3.zero,Vector3.zero,target);}
    #endregion
